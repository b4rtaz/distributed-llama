...
static inline float randomF32(unsigned long long *state) {
    // random float32 in <0,1)
    return (randomU32(state) >> 8) / 16777216.0f;
}

static inline void rmsnorm(float* o, const float* x, const float* weight, const int size) {
    assert(size % 4 == 0);
    float ss = 0.0f;
#if defined(__ARM_NEON)
    float32x4_t fsq;
...
static inline void softmax(float* x, const int size) {
    float maxVal;
#if defined(__ARM_NEON)
    float32x4_t fs;
